    ...
    public class LessonService : ILessonService
    {
        ...
        public async Task<ApiResponse> CreateNewLesson(CreateNewLessonRequest request) 
        {
            ...
            try
            {
                ...
                var checkExistLesson = await _unitOfWork.Lessons.GetAsync(l => l.CourseId == request.CourseId && l.ParentLessonId == request.ParentLessonId && l.Title.Trim().ToLower() == request.Title.Trim().ToLower());
                if (checkExistLesson != null) 
                {
                    return response.SetBadRequest(message: $"Lesson title '{request.Title}' already exists in this course");
                }
